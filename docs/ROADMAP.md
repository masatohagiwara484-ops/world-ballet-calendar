...
| # | Task | Status | Notes |
|---|------|--------|-------|
| 1 | Verified-dates trust badge | ✅ | `VerifiedDates` on perf page + cards. **Data-gated**: shows only when `last_verified` exists (Supabase). |
...
| # | Task | Status | Notes |
|---|------|--------|-------|
| 6 | Curated "Unmissable this season" rail | ✅ | `CuratedRail` on home; reuses `is_featured`. Max 12, typically 4–6. No dedicated page (by owner's call). Data-gated. |
| 7 | Leaflet venue map / "near me" | ✅ | `/map` explorer + per-house map on company pages + geolocation near-me. **Renders today** (company lat/lng exist). |
| 8 | "Plan the trip" panel | ✅ | `PlanYourTrip` on perf pages (hotels/experiences/tours). Monetizes once #2 IDs land. |

### C. Growth / 集客・成長
| # | Task | Status | Notes |
|---|------|--------|-------|
| 9 | Editorial long-tail SEO | ⬜ | Travel guides / work explainers / company stories. `/journal` + templates. Targets KWs Operabase ignores. |
| 10 | Auto social cards ("this week") | ⬜ | Beautiful shareable cards for this week's performances. Our unique visual distribution edge. |
...
> Note: #1 and #6 are **data-gated** (hidden until verified/featured rows exist),
> so the live site shows no visible change until **#3 (data)** is done. #7 renders
> immediately because company coordinates are already in the dataset.

---

## Priority order / 推奨優先順
1. **#3 Data verification & publishing** — unblocks the whole site (and makes
   #1/#6 visible). Without it, everything else is invisible to visitors.
2. **#9 Editorial long-tail SEO** — compounding, cheap, brand + funnel.
3. **#10 Auto social cards** — visual distribution / virality.
4. **#2 Affiliate IDs** — turn on revenue once traffic exists.
5. **#12 Ingestion automation** — scale coverage of the curated set.
6. **#11 Tests · #13 bios** — hardening.
