...
3. **作った bot とのDMを開き、[Start] を1回押す。** Telegram の仕様で、一度も
   Start を押していない相手に bot からメッセージを送ることはできません。ここを
   飛ばすと通知が1通も届きません。

> 🔐 **この数字は「あなたであること」の証明も兼ねます。** 承認ボタンを押した人の
> IDがこの番号と一致しない限り、公開処理は実行されません（第4-5節）。
...
✅ が3つ（トークン有効・チャット到達可能・Webhook登録済み）出れば完了です。
メッセージは1通も送られません。⚠️ が出たら、その行に対処法が書かれています。

### 4-6. 「誰が押したか」の確認（2重の鍵）

承認タップには**2つの独立したチェック**がかかります。

| チェック | 何を証明するか |
|---|---|
| 合言葉ヘッダー（`TELEGRAM_WEBHOOK_SECRET`） | 「**Telegram から**の通信である」 |
| 送信者ID（`TELEGRAM_CHAT_ID`） | 「**あなたが**押した」 |

合言葉だけでは不十分です。それは「Telegramから来た」ことしか示さず、「誰が押したか」
は分からないためです。**両方が未設定の場合は、誰も承認できません**（安全側に倒れる設計）。

> 将来、複数人でレビューする場合のみ `TELEGRAM_ALLOWED_USER_IDS`（カンマ区切りのID一覧）
> を設定すると、その全員が承認できるようになります。通常は設定不要です。

### 4-7. Telegram でできること一覧

| 操作 | 何が起きるか |
|---|---|
| **✅ Approve all** | まとめて公開。関連ページを自動更新し、フォロワーに通知 |
| **🚫 Reject all** | まとめて破棄。その館の自動承認カウントもリセット |
| **🔍 Details** | 会場・正確な日程・価格・チケットリンク・信頼度を1公演ずつ表示（6件ずつページ送り）。**↩︎ Summary** で戻る |
| **🔎 Open source** | 劇場の公式ページを開いて、承認前に自分の目で確認 |
| **`/pending` と入力** | 今たまっている未承認データを、いつでも呼び出す |
| **`/help` と入力** | bot の使い方を表示 |

> 🛡 **「✅ Approve all」は無条件の公開ではありません。** どの経路で承認しても、
> 次の3種類は自動的に**非公開のまま**保留され、承認後のメッセージに理由が表示されます。
>
> | 保留されるもの | 理由 |
> |---|---|
> | 公演中止（cancelled） | 中止は「行を隠す」ことで確定させる。中止情報をサイトに載せるわけではない |
> | ありえない日付 | 西暦0026年などの解析ミス、200日を超える「公演期間」（2公演の誤結合） |
> | 公演でないもの | 衣装販売・館内ガイドツアー・オープンクラスなど |
...
| Telegram の論理点検 | `npm run telegram:selftest`（bot もDBも不要） |
...
### 6-1. 巡回とは無関係に「今の未承認一覧」を見る

巡回通知は「そのとき見つかった変更」しか送りません。**今キューに残っているもの**を
確認したいときは、Telegram で **`/pending`** と打つだけです（ターミナル不要）。

パソコンから送りたい場合は同じ処理をコマンドでも呼べます。

```bash
npm run review:telegram                          # 未承認がある全劇場ぶん送信
npm run review:telegram -- --slug royal-ballet   # 1劇場だけ
npm run review:telegram -- --dry                 # 件数を数えるだけ（送信しない）
```

劇場ごとに**まとめは1つ**（`pending:<slug>`）なので、何度呼び出しても通知が
たまり続けることはありません。最新の内容に上書きされます。

ターミナルで一覧を見たい場合は従来どおり `npm run review:pending` が使えます。
どちらの経路でも公開時の安全チェック（第4-7節）は同一です。
